...
// MulFn is an operation that complies with the Function interface
type MulFn[T Number] struct {
    Ctx *Context[T]
}
func (op *MulFn[T]) Forward(args []any, inputs ...*Tensor[T]) (*Tensor[T], error) {
    if len(inputs) != 2 {
        return nil, fmt.Errorf("Add operation requires two tensors, got %d", len(inputs))
    }
    x, y := inputs[0], inputs[1]
...
func (op *MulFn[T]) Backward(gradOutput *Tensor[T]) (*Tensor[T], error) {
    // use the stored context
    // ctx := op.Ctx
    // this is how you retrieve saved data
    // x := ctx.SavedShapes[0].([]int)
